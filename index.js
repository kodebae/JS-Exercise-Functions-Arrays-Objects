 ...
 */
function temperatureCtoF(cel) { // pass in arguments for cel
    let cToF = Math.round(cel * 9 / 5 + 32); //create the math conversion and round
    return cToF; //return the variable with the converted data
}
 ...
 * 
 * Hint: You can call your `
    temperatureCtoF ` function from inside `
    temperatureInF `.
 */
function temperatureInF(temperature, unit) { // add temp and unit parameters
    if (unit === 'C') {
        let cToF = Math.round(temperature * 9 / 5 + 32); //create the math conversion and round
        return cToF; //return the variable with the converted data
    } else if (unit === 'F') {
 ...
 * an object containing a `
    name ` property,
 * and return a string that reads `
    Hello, my name is { name }
    `,
 * where ` { name }
    ` is the name stored in the object.
 * 
 * For example, if we invoke `
    getName `
